...
test("stations 1-4 open on a briefing, station 5 does not", () => {
  assert.equal(hasBriefing(0), true);
  assert.equal(hasBriefing(3), true);
  assert.equal(hasBriefing(4), false);
...
test("advancing walks the whole demo and stops at the last station", () => {
  ...
  assert.deepEqual([...new Set(visited)], [0, 1, 2, 3, 4]);
  assert.equal(state.station, STATION_COUNT - 1);
});

test("SPEC.md idle rule: 45 s on station 5, 90 s everywhere else", () => {
  assert.equal(idleTimeoutMs(station(4)), IDLE_MS_STATION_5);
  assert.equal(IDLE_MS_STATION_5, 45_000);
  for (const n of [0, 1, 2, 3] as StationIndex[]) {
    assert.equal(idleTimeoutMs(station(n)), IDLE_MS);
  }
  assert.equal(IDLE_MS, 90_000);
  // The station-5 briefing does not exist, but its diploma must reset sooner
...
test("restart returns to the attract screen from anywhere", () => {
  assert.deepEqual(flowReducer(station(4), { type: "restart" }), initialFlow());
});
